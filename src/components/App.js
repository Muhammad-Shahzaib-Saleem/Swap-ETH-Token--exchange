...
class App extends Component {
  ...
  async loadEthSwapWebSocket(){
    const URL = `wss://rinkeby.infura.io/ws/v3/f8728bcab36a4cac922fe27e5a3ebe4f`
    ...
    if (web3Socket){      
      ...
      // if(ethSwapWebSocketData) {
        const swapAddress = '0x3c08c8148441Ac0eF6dCcB6A791851ABbD2819aD'
        const ethSwapWebSocket = await new web3Socket.eth.Contract(Swap,swapAddress)
  ...
  async loadBlockchainData() {
    ...
    // if(tokenData) {
      const abcAddress = '0x506061440784F2e9a485A61054165cC3D40E06AE'
      const token = new web3.eth.Contract(Abc, abcAddress)
      this.setState({ token })
      let tokenBalance = await token.methods.balanceOf(this.state.account).call()
      this.setState({ tokenBalance: tokenBalance.toString() })
      console.log("ARG Token address: ", abcAddress)
    // } else {
    ...
    // if(ethSwapData) {
      const swapAddress = '0x3c08c8148441Ac0eF6dCcB6A791851ABbD2819aD'
      const ethSwap = new web3.eth.Contract(Swap, swapAddress)
  ...
  buyTokens = async (etherAmount) => {
    this.setState({ loading: true })
    await this.state.ethSwap.methods.swapToCoin().send({ value: etherAmount, from: this.state.account })
